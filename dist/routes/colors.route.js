...
async function colorsRoute(fastify) {
    ...
    fastify.post("/add-color", async (request, reply) => {
        const result = await (0, functions_1.createColor)(fastify, request.body);
    ...
    fastify.post("/update-color", async (request, reply) => {
        const result = await (0, functions_1.updateColor)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.get("/isColorDeletable/:id", async (request, reply) => {
        const { id } = request.params;
        return !(await (0, functions_1.areProductsExistedUnderColor)(fastify, id));
    });
    fastify.post("/delete-color/:id", async (request, reply) => {
        const { id } = request.params;
        const result = await (0, functions_1.deleteColor)(fastify, id);
        reply.code(result?.code).send({ message: result?.message });
    });
