...
async function productsSideNavsRoute(fastify) {
    fastify.get("/all-products-sideNavs", async (request, reply) => {
        return await (0, bo_functions_1.getAllProductsSideNavs)(fastify);
    });
    fastify.get("/all-main-products-sideNavs", async (request, reply) => {
        return await (0, bo_functions_1.getMainProductsSideNavs)(fastify);
    });
    ...
    fastify.post("/update-products-sideNav", async (request, reply) => {
        const result = await (0, bo_functions_1.updateProductsSideNav)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.post("/delete-products-sideNav/:id", async (request, reply) => {
        const { id } = request.params;
        const result = await (0, bo_functions_1.deleteSideNav)(fastify, id);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.post("/delete-sub-products-sideNav", async (request, reply) => {
        const result = await (0, bo_functions_1.deleteSubSideNavs)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
}
