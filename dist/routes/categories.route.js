...
async function categoriesRoute(fastify) {
    ...
    fastify.get("/all-categories-no-level", async (request, reply) => {
        return await (0, functions_1.getAllCategoriesNoLevel)(fastify);
    });
    fastify.get("/main-categories-no-sub", async (request, reply) => {
        return await (0, functions_1.getMainCategoriesWithoutSub)(fastify);
    });
    ...
    fastify.post("/add-sub-categories", async (request, reply) => {
        const result = await (0, functions_1.addSubCategories)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
    ...
    fastify.get("/isCategoryDeletable/:id", async (request, reply) => {
        const { id } = request.params;
        return !(await (0, functions_1.areProductsExistedUnderCategory)(fastify, id));
    });
    ...
    fastify.post("/delete-sub-categories", async (request, reply) => {
        const result = await (0, functions_1.deleteSubCategories)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
