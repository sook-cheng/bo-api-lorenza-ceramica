...
async function inspirationsRoute(fastify) {
    ...
    fastify.post("/add-inspiration", async (request, reply) => {
        const result = await (0, bo_functions_1.createInspiration)(fastify, request.body, await request.file({ limits: { fileSize: 100000 } }));
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.post("/update-inspiration", async (request, reply) => {
        const result = await (0, bo_functions_1.updateInspiration)(fastify, request.body, await request.file({ limits: { fileSize: 100000 } }));
        reply.code(result?.code).send({ message: result?.message });
    });
