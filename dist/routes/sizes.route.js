...
async function sizesRoute(fastify) {
    ...
    fastify.post("/add-size", async (request, reply) => {
        const result = await (0, functions_1.createSize)(fastify, request.body);
    ...
    fastify.post("/update-size", async (request, reply) => {
        const result = await (0, functions_1.updateSize)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.get("/isSizeDeletable/:id", async (request, reply) => {
        const { id } = request.params;
        return !(await (0, functions_1.areProductsExistedUnderSize)(fastify, id));
    });
    fastify.post("/delete-size/:id", async (request, reply) => {
        const { id } = request.params;
        const result = await (0, functions_1.deleteSize)(fastify, id);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.post("/delete-sizes", async (request, reply) => {
        const result = await (0, functions_1.deleteSizes)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
}
