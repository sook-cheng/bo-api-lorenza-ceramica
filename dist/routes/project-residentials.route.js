...
async function projectResidentialsRoute(fastify) {
    fastify.get("/all-project-residentials", async (request, reply) => {
        return await (0, functions_1.getAllProjectResidentials)(fastify);
    });
    fastify.get("/project-residential-details/:id", async (request, reply) => {
        const { id } = request.params;
        return await (0, functions_1.getProjectResidentialDetailsById)(fastify, id);
    });
    fastify.post("/add-project-residential", async (request, reply) => {
        const result = await (0, functions_1.createProjectResidential)(fastify, request.body);
    ...
    fastify.post("/update-project-residential", async (request, reply) => {
        const result = await (0, functions_1.updateProjectResidential)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.post("/delete-project-residential/:id", async (request, reply) => {
        const { id } = request.params;
        const result = await (0, functions_1.deleteProjectResidential)(fastify, id);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.post("/delete-project-residentials", async (request, reply) => {
        const result = await (0, functions_1.deleteProjectResidentials)(fastify, request.body);
        reply.code(result?.code).send({ message: result?.message });
    });
    fastify.post("/upload-residential-thumbnail/:id", async (request, reply) => {
        const { id } = request.params;
        const image = await request.file({ limits: { fileSize: 10000000 } });
        const result = await (0, functions_1.uploadResidentialThumbnail)(fastify, id, image);
        reply.code(result?.code).send({ message: result?.message });
    });
    ...
    fastify.get("/project-residentials-images/:id", async (request, reply) => {
        const { id } = request.params;
        return await (0, functions_1.getProjectResidentialsImagesById)(fastify, id);
    });
}
